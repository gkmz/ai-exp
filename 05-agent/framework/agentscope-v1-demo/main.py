...
class ThreeKingdomsWerewolfGame:
    ...
    async def werewolf_phase(self, round_num: int) -> str | None:
        ...
        async with MsgHub(
            ...
            announcement=await self.moderator.announce(
                f"狼人们，请讨论今晚的击杀目标。存活玩家：{util.format_player_list(self.alive_players)}"
            ),
        ) as werewolves_hub:
            # 讨论阶段
    ...
    async def day_phase(self, round_num: int):
        ...
        async with MsgHub(
            ...
            # 每人发言一轮, 等待整轮结束
            current_alive_players: list[AgentBase] = list(self.alive_players)
            await sequential_pipeline(current_alive_players)

            # 投票阶段
            all_hub.set_auto_broadcast(False)
            vote_msgs = await fanout_pipeline(
                current_alive_players,
