...
class SearchState(TypedDict):
    # 更新规则交给 add_messages 方法来处理
    # - 新消息追加到已有消息列表末尾
    # - 如果新旧消息拥有相同的 `id`，则更新原消息，而不是重复追加
    # - 自动把字典格式的消息转换成 LangChain 的 `HumanMessage`、`AIMessage` 等对象 ([langchain-ai.github.io](https://langchain-ai.github.io/langgraph/how-tos/state-reducers/?utm_source=openai))
...
def understand_query_node(state: SearchState) -> SearchState:
    ...
    # 获取最新的用户消息
    # state["messages"]保存的是所有消息历史，新消息在最后，所以需要反转再循环读取，找到最新一条的人输入消息
    user_message = ""
...
def create_search_assistant():
    ...
    # 设置线性流程, start -> understand -> search -> answer -> end
